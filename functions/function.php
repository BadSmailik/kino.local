...
function selectAll($nameDB)
{
    $select = R::findOne('users', [$nameDB]);
    if ($select) {
        return $select;
    }
}
...
function actionComment($id_content)
{
    ...
    if ($_SESSION['user'] == NULL) {
        $_SESSION['msg'] = 'Сначала нужно&nbsp;<b><a href="register">зарегестрироваться!</a></b>';
    }
    if (empty($_POST['comment'])) {
        $_SESSION['msg'] = '<div class="text-danger">Заполните поле комментарий</div>';
    } else {
